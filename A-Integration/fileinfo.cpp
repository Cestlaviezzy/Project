...
int fileinfo::ReadSetINI(CString path)
{
	CString stra;
	int n;
	GetPrivateProfileString(_T("ReverseTest"), _T("Time"), CString("NULL"), stra.GetBuffer(MAX_PATH), MAX_PATH, path);
	n = _ttoi(stra);
	return n;
}

int fileinfo::SendToMESWaitTime(CString path)
{
	CString stra;
	int n;
	GetPrivateProfileString(_T("SendToMES"), _T("Time"), CString("1"), stra.GetBuffer(MAX_PATH), MAX_PATH, path);
	n = _ttoi(stra);
	return n;
}